...
{
    internal class PlanarProjectorInputReadable : MonoBehaviour,
        IInputReadable<float>, IInputReadable<Vector2>, IInputReadable<Vector3>, IInputReadable<Vector4>
    {
        ...
        public Vector4 ProjectionNormal { get; set; } = Vector3.up;
        ...
        {
            Vector3 projectedInput = Vector3.ProjectOnPlane(new Vector3(_inputReadableFloat.GetInput(), 0.0f, 0.0f), ProjectionNormal);
            return _renormalize ? Mathf.Sign(projectedInput.x) : projectedInput.x;
        }

        Vector2 IInputReadable<Vector2>.GetInput()
        {
            Vector2 projectedInput = Vector3.ProjectOnPlane(_inputReadableVector2.GetInput(), ProjectionNormal);
            return _renormalize ? projectedInput.normalized : projectedInput;
        }

        Vector3 IInputReadable<Vector3>.GetInput()
        {
            Vector3 projectedInput = Vector3.ProjectOnPlane(_inputReadableVector3.GetInput(), ProjectionNormal);
            return _renormalize ? projectedInput.normalized : projectedInput;
        }

        Vector4 IInputReadable<Vector4>.GetInput()
        {
            Vector4 projectedInput = Vector4.Project(_inputReadableVector4.GetInput(), ProjectionNormal);
            return _renormalize ? projectedInput.normalized : projectedInput;
        }
    }
}