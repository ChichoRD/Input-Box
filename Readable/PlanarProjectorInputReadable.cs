...
public class PlanarProjectorInputReadable : MonoBehaviour,
    IInputReadable<float>, IInputReadable<Vector2>, IInputReadable<Vector3>, IInputReadable<Vector4>
{
    ...
    public float Get()
    {
        Vector3 projectedInput = Vector3.ProjectOnPlane(new Vector3(_inputReadableFloat.Get(), 0.0f, 0.0f), ProjectionNormal);
        return _renormalize ? Mathf.Sign(projectedInput.x) : projectedInput.x;
    }

    Vector2 IInputReadable<Vector2>.Get()
    {
        Vector2 projectedInput = Vector3.ProjectOnPlane(_inputReadableVector2.Get(), ProjectionNormal);
        return _renormalize ? projectedInput.normalized : projectedInput;
    }

    Vector3 IInputReadable<Vector3>.Get()
    {
        Vector3 projectedInput = Vector3.ProjectOnPlane(_inputReadableVector3.Get(), ProjectionNormal);
        return _renormalize ? projectedInput.normalized : projectedInput;
    }

    Vector4 IInputReadable<Vector4>.Get()
    {
        Vector4 projectedInput = Vector4.Project(_inputReadableVector4.Get(), ProjectionNormal);
        return _renormalize ? projectedInput.normalized : projectedInput;
    }
}